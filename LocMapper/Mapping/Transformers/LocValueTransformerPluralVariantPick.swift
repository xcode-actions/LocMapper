...
final class LocValueTransformerPluralVariantPick : LocValueTransformer {
	...
	override func apply(toValue value: String, withLanguage language: String) throws -> String {
		...
		if Set(["chinese", "japanese", "thai"]).contains(where: { language.range(of: $0) != nil }) {
			...
			/* Technically, for French, Hindi and Brazilian Portuguese, the plurality definition is "(0:1)(*)",
			 * but as we use 1 and 2 for the values of n,
			 * we don't care about the difference in the 0 case for these two languages! */
			...
			/* Note: We do not require the full plurality definition here as we use static values when resolving the string…
			 *       Let's put it anyway for reference. */
