...
public final class LocFile {
	...
	internal var entries: [LineKey: LineValue] {
		didSet {
			/* We could probably be more precise on cache invalidation and invalidate only what’s needed,
			 * but it would have to be done when setting the new values in the entries directly
			 * and we might miss some places.
			 * We do not really need to care about the cache much, so we invalidate it any time the entries are modified. */
			invalidateCache()
		}
	}
	...
	/**
	 Remove the references to a language from the given path and add the language found in the list of known languages in the LocFile.
	 
	 - Returns: A tuple of the language agnostic filename and the found language name. */
	...
	/**
	 ...
	 If the key is not already present in the file, simply add it to the keys in the file and return it.
	 
	 If the key is already in the file, try and merge the comments and user info from the key given in argument and return the merged key, after updating the keys registered in the file.
	 
	 - Returns: Either the original key, or a merge of the corresponding existing key in the already known keys with the one given in argument. */
		...
		if let idx = keys.firstIndex(of: refKey) {
			if keys[idx].comment != refKey.comment {
				if useNonEmptyCommentIfOneEmptyTheOtherNot && (keys[idx].comment.isEmpty || refKey.comment.isEmpty) {
					/* We use the non-empty comment because one of the two comments compared is empty; the other not (both are different and one of them is empty). */
...
#if canImport(os)
					Conf.oslog.flatMap{ os_log("Got different comment for same loc key \"%@\" (file %@): \"%@\" and \"%@\"", log: $0, type: .info, refKey.locKey, refKey.filename, keys[idx].comment, refKey.comment) }
#endif
					Conf.logger?.info("Got different comment for same loc key \"\(refKey.locKey)\" (file \(refKey.filename): \"\(keys[idx].comment)\" and \"\(refKey.comment)\"")
				}
			}
			return keys[idx]
		}
